...
class SmartcarSplitter extends IPSModuleStrict
{
    ...
    public function BuildConnectURL(string $mode, string $state, array $permissions, string $vehicleId = '', bool $reauthenticate = false): string
    {
        ...
        $isVehicleReauthorization = $reauthenticate && trim($vehicleId) !== '';

        // Smartcar Connect:
        // - Erstverbindung: response_type=code
        // - Reauthorization eines bestehenden Fahrzeugs:
        //   response_type=<VehicleID>
        //
        // Wichtig: Für den Connect-Flow wird application_id verwendet.
        $query = [
            'application_id' => $applicationId,
            'response_type'  => $isVehicleReauthorization ? trim($vehicleId) : 'code',
