...
class SmartcarSplitter extends IPSModuleStrict {
    ...
    public function Create(): void {
        ...
        $hookPath='/hook/'.$hookAddress;
        $manualRedirectURI=trim($this -> ReadPropertyString('ManualRedirectURI'));
            $form=['elements' =>[['type' => 'Label', 'caption' => 'Lokaler Smartcar Hook-Pfad: '.$hookPath], ['type' => 'Label', 'caption' => $redirectURI !== ''?'Aktuelle Redirect-/Webhook-URL: '.
            $redirectURI: 'Aktuelle Redirect-/Webhook-URL: (leer – Symcon Connect nicht gefunden)'], ['type' => 'Label', 'caption' => $manualRedirectURI !== ''?'URL-Quelle: Manuelle Redirect-/Webhook-URI': 'URL-Quelle: Automatisch über Symcon Connect']
            , ['type' => 'Label', 'caption' => 'Cloudflare Tunnel: Die öffentliche URL muss auf diesen Hook zeigen, z. B. '.'https://deine-domain.tld'.$hookPath],
            ['type' => 'ValidationTextBox', 'name' => 'ManualRedirectURI', 'caption' => 'Redirect-/Webhook-URI manuell überschreiben'], ['type' => 'ValidationTextBox',
            'name' => 'ClientID', 'caption' => 'Client ID'], ['type' => 'ValidationTextBox', 'name' => 'ClientSecret', 'caption' => 'Client Secret'], ['type' => 'ValidationTextBox',
            'name' => 'ApplicationID', 'caption' => 'Application ID für Smartcar Connect'], ['type' => 'ValidationTextBox', 'name' => 'ManagementToken', 'caption' => 'Application Management Token']
        ]];
        ...
        $postData=http_build_query(['grant_type' => 'client_credentials', 'client_id' => $clientID, 'client_secret' => $clientSecret]);
        $response=$this -> HttpRequestRaw('Token', 'POST', $url, ['Content-Type: application/x-www-form-urlencoded', 'Accept: application/json'], $postData);
        if($response === null) {
            return false;
        ...
        switch($command) {
            case 'LoadConnections': return json_encode($this -> LoadConnections(), JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);
            case 'BuildConnectURL': $url=$this -> BuildConnectURL((string)($data['Mode']?? 'live'), (string)($data['State']?? ''));
            $success=str_starts_with($url, 'https://');
            return json_encode(['success' => $success, 'url' => $success?$url: '', 'error' => $success?'': $url, 'flow' => 'authorize'], JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);
            case 'BuildAuthorizationURL': $vehicleId=trim((string)($data['VehicleID']?? ''));
            $state=(string)($data['State']?? '');
            $mode=(string)($data['Mode']?? 'live');
            if($vehicleId !== '') {
                $url=$this -> BuildReauthURL($vehicleId, $state);
                $flow='reauthenticate';
            }else {
                $url=$this -> BuildConnectURL($mode, $state);
                $flow='authorize';
            }$success=str_starts_with($url, 'https://');
            return json_encode(['success' => $success, 'url' => $success?$url: '', 'error' => $success?'': $url, 'flow' => $flow], JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);
            case 'BuildReauthURL': $url=$this -> BuildReauthURL((string)($data['VehicleID']?? ''), (string)($data['State']?? ''));
            $success=str_starts_with($url, 'https://');
            return json_encode(['success' => $success, 'url' => $success?$url: '', 'error' => $success?'': $url, 'flow' => 'reauthenticate'], JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);
            case 'GetVehicle': return json_encode($this -> ApiGetVehicle((string)($data['VehicleID']?? ''), (string)($data['UserID']?? '')), JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);
            case 'GetSignals': return json_encode($this -> ApiGetSignals((string)($data['VehicleID']?? ''), (string)($data['UserID']?? '')), JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);
                case 'GetSignal': return json_encode($this -> ApiGetSignal((string)($data['VehicleID']?? ''), (string)($data['SignalCode']?? ''), (string)($data['UserID']?? '')),
            JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);
                case 'Command': return json_encode($this -> ApiVehicleCommand((string)($data['VehicleID']?? ''), (string)($data['Method']?? 'GET'), (string)($data['Path']?? ''),
            isset($data['Body'])?json_encode($data['Body']): '', (string)($data['UserID']?? '')), JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);
                case 'GetCompatibleVehicles': return json_encode($this -> ApiGetCompatibleVehicles((string)($data['Make']?? ''), (string)($data['PowertrainType']?? ''),
            (string)($data['Region']?? 'EUROPE')), JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);
            default: return json_encode(['success' => false, 'error' => 'Unknown command']);
        }
        ...
        $response=$this -> HttpRequestRaw('Connections', 'GET', $url, ['Authorization: Bearer '.$token, 'Accept: application/json']);
        ...
        foreach($data['data']as $item) {
            ...
            if($caption === '') {
                $caption=$vehicleId;
                }$connections[]=['connectionId' => $connectionId, 'vehicleId' => $vehicleId, 'userId' => $userId, 'caption' => $caption, 'make' => $make, 'model' => $model,
            'year' => $year, 'mode' => $modeValue, 'powertrainType' => $powertrainType, 'permissions' => $attributes['permissions']??[]];
        }return $connections;
        ...
        return $this -> ApiRequest('GET', '/vehicles/'.rawurlencode($vehicleId).'/signals/'.rawurlencode($signalCode), null, $userId);
        ...
        return $this -> ApiRequest($method, '/vehicles/'.rawurlencode($vehicleId).$path, $decodedBody, $userId);
        ...
        if(is_array($response)) {
            $this -> SendDebug('ApiRequest/Headers', json_encode($response['headers']??[], JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE), 0);
        }if($response === null) {
            return['success' => false, 'error' => 'No response'];
        }$decoded=json_decode($response['body'], true);
            return['success' => $response['statusCode']>= 200 && $response['statusCode']<300, 'statusCode' => $response['statusCode'], 'headers' => $response['headers'],
        ...
        $headerText=implode("\r\n", $headers)."\r\n";
        $options=['http' =>['method' => strtoupper($method), 'header' => $headerText, 'ignore_errors' => true, 'timeout' => 30]];
        if($content !== null) {
        ...
        return['statusCode' => $statusCode, 'headers' => $responseHeaders, 'body' => $body];
        ...
        }$response=$this -> HttpRequestRaw('Compatibility', 'GET', $url, ['Accept: application/json']);
        if($response === null) {
            return['success' => false, 'error' => 'No response'];
        }$this -> SendDebug('Connections/RAW', $response['body'], 0);
        $decoded=json_decode($response['body'], true);
        return['success' => $response['statusCode']>= 200 && $response['statusCode']<300, 'statusCode' => $response['statusCode'], 'body' => is_array($decoded)?$decoded: $response['body']];
        ...
        if($eventType === 'VERIFY') {
            ...
            echo 'Unauthorized';
            return;
        }$vehicleId=(string)($payload['vehicleId']?? $payload['data']['vehicle']['id']?? $payload['data']['vehicleId']?? '');
            $this -> SendDebug('Webhook/Event', json_encode(['eventType' => $eventType, 'vehicleId' => $vehicleId, 'eventId' => $payload['eventId']?? '', 'meta' => $payload['meta']??[]],
        JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE), 0);
        switch($eventType) {
            case 'VEHICLE_STATE': $triggers=is_array($payload['triggers']?? null)?$payload['triggers']: (is_array($payload['data']['triggers']?? null)?$payload['data']['triggers']: []);
            ...
            foreach($triggers as $trigger) {
                ...
                $this -> SendDebug('Webhook/FIRST_DELIVERY', 'Synchronisiere Connections für automatische Vehicle-Instanz.', 0);
            ...
            echo 'ok';
            return;
            case 'VEHICLE_ERROR': $this -> HandleVehicleErrorWebhook($payload);
            http_response_code(200);
            echo 'ok';
            return;
            default: $this -> SendDebug('Webhook/UnknownEvent', 'Unbekannter eventType='.$eventType, 0);
            http_response_code(200);
            echo 'ok';
            return;
        }
    ...
    /**
         ...
         */
    public function BuildConnectURL(string $mode, string $state): string {
        ...
        if($mode !== 'simulated') {
            ...
            try {
                $state='connect_'.bin2hex(random_bytes(12));
            }catch(Throwable $e) {
                $state='connect_'.uniqid('', true);
            }
        }$query=['response_type' => 'none', 'application_id' => $applicationID, 'redirect_uri' => $redirectURI, 'state' => $state, 'mode' => $mode];
        $url='https://connect.smartcar.com/oauth/authorize?'.http_build_query($query, '', '&', PHP_QUERY_RFC3986);
            $this -> SendDebug('ConnectURL/Build', json_encode(['flow' => 'authorize', 'mode' => $mode, 'responseType' => 'none', 'applicationId' => $applicationID,
        'permissionSource' => 'vehicle_access_dashboard', 'url' => $url], JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE), 0);
        return $url;
    }
    /**
         ...
         */
    public function BuildReauthURL(string $vehicleId, string $state): string {
        $applicationID=trim($this -> ReadPropertyString('ApplicationID'));
        $redirectURI=$this -> GetRedirectURI();
        if($applicationID === '') {
            return 'Fehler: Application ID für Smartcar Connect fehlt.';
        ...
        if($vehicleId === '') {
            return 'Fehler: Vehicle ID fehlt.';
        }if($state === '') {
            try {
                $state='reauth_'.$vehicleId.'_'.bin2hex(random_bytes(8));
            }catch(Throwable $e) {
                $state='reauth_'.$vehicleId.'_'.uniqid('', true);
            }
        }$query=['response_type' => 'vehicle_id', 'application_id' => $applicationID, 'vehicle_id' => $vehicleId, 'redirect_uri' => $redirectURI, 'state' => $state];
        $url='https://connect.smartcar.com/oauth/reauthenticate?'.http_build_query($query, '', '&', PHP_QUERY_RFC3986);
            $this -> SendDebug('ReauthURL/Build', json_encode(['flow' => 'reauthenticate', 'vehicleId' => $vehicleId, 'responseType' => 'vehicle_id', 'applicationId' => $applicationID,
        'permissionSource' => 'vehicle_access_dashboard', 'url' => $url], JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE), 0);
        return $url;
        ...
        return $this -> BuildSymconConnectURL($hookPath);
        ...
        $connectAddress='';
        $ids=@IPS_GetInstanceListByModuleID(self :: CONNECT_CONTROL_MODULE_ID);
        ...
        $redirectVehicleId=(string)($_GET['vehicle_id']??($_GET['vehicleId']?? ''));
            $this -> SendDebug('Connect/Redirect', json_encode(['code' => $code !== ''?'<present>': '', 'vehicle_id' => $redirectVehicleId, 'user_id' => $userId, 'state' => $state],
        JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE), 0);
        /*
                 ...
                 */
        $vehicleId=$redirectVehicleId !== ''?$redirectVehicleId: $this -> ExtractVehicleIdFromState($state);
        if($vehicleId !== '' && $userId !== '') {
        ...
        $attempt ++) {
            ...
            $this -> SendDebug('Connect/SyncVehicles', 'Versuch '.$attempt.'/'.$maxAttempts.', Connections='.count($connections), 0);
        ...
        }$this -> SendDebug('Connect/SyncVehicles', 'Nach '.$maxAttempts.' Versuchen keine Connection gefunden.', 0);
        ...
        foreach($connections as $connection) {
            ...
            if($instanceId === 0) {
                ...
                $isNew=true;
                $this -> SendDebug('Connect/CreateVehicle', 'Neue Vehicle-Instanz erstellt: '.$instanceId.' für VehicleID='.$vehicleId, 0);
        ...
        if(preg_match('/(?:vehicle|reauth)_([0-9a-fA-F-]{36})_/', $state, $m)) {
            return $m[1];
        }return '';
    }private function HandleVehicleErrorWebhook(array $payload): void {
        $data=is_array($payload['data']?? null)?$payload['data']: [];
        $vehicleId=(string)($payload['vehicleId']?? $data['vehicle']['id']?? $data['vehicleId']?? '');
        $err=$data['error']??($data['errors'][0]?? $data);
        if(!is_array($err)) {
            $err=[];
            }$this -> SendDebug('VEHICLE_ERROR', json_encode(['eventId' => $payload['eventId']?? '', 'vehicleId' => $vehicleId, 'type' => $err['type']?? '', 'code' => $err['code']?? '',
            'description' => $err['description']??($err['message']?? ''), 'requestId' => $err['requestId']??($err['requestID']?? ''), 'docURL' => $err['docURL']??($err['docUrl']?? ''),
        'resolution' => $err['resolution']?? null, 'meta' => $payload['meta']??[]], JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE), 0);
    }
}
